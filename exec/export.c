/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   export.c                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ababouel <ababouel@student.1337.ma>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/21 15:41:51 by sismaili          #+#    #+#             */
/*   Updated: 2022/07/23 18:29:39 by ababouel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/exec.h"

static t_val	*equal_export(t_cmd *cmd, int i)
{
	char	*temp;
	t_val	*result;

	temp = NULL;
	result = search_val(cmd->env->head, strrchr(cmd->cmdarg[i], '='));
	if (result)
	{
		temp = result->value;
		result->value = ft_strdup(cmd->cmdarg[i]);
		free(temp);
		temp = NULL;
		return (NULL);
	}
	result = add_val(cmd->cmdarg[i]);
	return (result);
}

static t_val	*plus_export(t_cmd *cmd, int i)
{
	char	*temp;
	t_val	*result;

	temp = NULL;
	result = search_val(cmd->env->head, strrchr(cmd->cmdarg[i], '='));
	if (result)
	{
		temp = strchr(cmd->cmdarg[i], '=');
		result->value = ft_strjoinbis(result->value, &temp[1]);
		return (NULL);
	}
	temp = strrchr(cmd->cmdarg[i], '+');
	temp = ft_strjoin(temp, strchr(cmd->cmdarg[i], '='));
	result = add_val(temp);
	free(temp);
	temp = NULL;
	return (result);
}

static void	check_export(t_cmd *cmd, int i)
{
	int		j;
	int		check;
	t_val	*result;

	j = 0;
	check = 0;
	result = NULL;
	while (cmd->cmdarg[i][j])
	{
		if ((cmd->cmdarg[i][j] == '=' || (cmd->cmdarg[i][j] == '+'
			&& cmd->cmdarg[i][j + 1] == '=')) && check == 0)
		{
			if (cmd->cmdarg[i][j] == '+')
			{
				result = plus_export(cmd, i);
				if (result)
					add_node(cmd->env, result);
				result = plus_export(cmd, i);
				if (result)
					add_node(cmd->export, result);
				return ;
			}
			else
			{
				result = equal_export(cmd, i);
				if (result)
					add_node(cmd->env, result);
				result = equal_export(cmd, i);
				if (result)
					add_node(cmd->export, result);
				return ;
			}
		}
		if (!ft_isalpha(cmd->cmdarg[i][j]) && cmd->cmdarg[i][j] != '_'
			&& !ft_isdigit(cmd->cmdarg[i][j]))
			check = 1;
		j++;
	}
	result = exportenv(cmd, i, j, check);
	if (result)
		add_node(cmd->export, result);
}

int	ft_export(t_cmd *cmd)
{
	int	i;

	i = 1;
	if (!cmd->cmdarg[i])
	{
		sorting(cmd->export->head);
		return (0);
	}
	while (cmd->cmdarg[i])
	{
		if (ft_isalpha(cmd->cmdarg[i][0]) || cmd->cmdarg[i][0] == '_')
		{
			if (!(cmd->cmdarg[i][0] == '_' && cmd->cmdarg[i][1] == '='))
				check_export(cmd, i);
		}
		else
			return (printf("export: `%s': not a valid identifier\n",
					cmd->cmdarg[i]), 1);
		i++;
	}
	return (0);
}
